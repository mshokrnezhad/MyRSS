# Spotify Engineering Blog

## Encoding Your Domain Expert: The Context Layer Behind Spotify's Data Assistant
- **Link**: https://engineering.atspotify.com/2026/6/encoding-your-domain-expert-the-context-layer-behind-spotifys-data-assistant
- **Description**: Encoding Your Domain Expert: The Context Layer Behind Spotify's Data Assistant
- **First seen**: 2026-07-05

## Coding Is No Longer the Constraint: Scaling Developer Experience to Teams and Agents at Spotify
- **Link**: https://engineering.atspotify.com/2026/6/code-with-claude-coding-is-no-longer-the-constraint
- **Description**: Coding Is No Longer the Constraint: Scaling Developer Experience to Teams and Agents at Spotify
- **First seen**: 2026-07-05

## Better Experiments with LLM Evals — A funnel, not a fork
- **Link**: https://engineering.atspotify.com/2026/5/better-experiments-with-llm-evals-a-funnel-not-a-fork
- **Description**: Better Experiments with LLM Evals — A funnel, not a fork
- **First seen**: 2026-07-05

## Building a Natural Language Interface to the Spotify Ads API with Claude Code Plugins
- **Link**: https://engineering.atspotify.com/2026/5/spotify-ads-api-claude-plugins
- **Description**: Building a Natural Language Interface to the Spotify Ads API with Claude Code Plugins
- **First seen**: 2026-07-05

## Background Coding Agents: Supercharging Downstream Consumer Dataset Migrations (Honk, Part 4)
- **Link**: https://engineering.atspotify.com/2026/4/background-coding-agents-dataset-migrations-honk-part-4
- **Description**: Background Coding Agents: Supercharging Downstream Consumer Dataset Migrations (Honk, Part 4)
- **First seen**: 2026-07-05

## Let’s Talk Agentic Development: Spotify x Anthropic Live
- **Link**: https://engineering.atspotify.com/2026/4/anthropic-agentic-development
- **Description**: Let’s Talk Agentic Development: Spotify x Anthropic Live
- **First seen**: 2026-07-05

## Inside the Archive: The Tech Behind Your 2025 Wrapped Highlights
- **Link**: https://engineering.atspotify.com/2026/3/inside-the-archive-2025-wrapped
- **Description**: Inside the Archive: The Tech Behind Your 2025 Wrapped Highlights
- **First seen**: 2026-07-05

## Our Multi-Agent Architecture for Smarter Advertising
- **Link**: https://engineering.atspotify.com/2026/2/our-multi-agent-architecture-for-smarter-advertising
- **Description**: Our Multi-Agent Architecture for Smarter Advertising
- **First seen**: 2026-07-05

## How We Release the Spotify App: A Look Under the Hood (Part 2)
- **Link**: https://engineering.atspotify.com/2026/2/how-we-release-the-spotify-app-part-2
- **Description**: How We Release the Spotify App: A Look Under the Hood (Part 2)
- **First seen**: 2026-07-05

## Congratulations to the recipients of the 2025 Spotify FOSS Fund
- **Link**: https://engineering.atspotify.com/2026/1/congratulations-to-the-recipients-of-the-2025-spotify-foss-fund
- **Description**: Congratulations to the recipients of the 2025 Spotify FOSS Fund
- **First seen**: 2026-07-05

## Why We Use Separate Tech Stacks for Personalization and Experimentation
- **Link**: https://engineering.atspotify.com/2026/1/why-we-use-separate-tech-stacks-for-personalization-and-experimentation
- **Description**: Why We Use Separate Tech Stacks for Personalization and Experimentation
- **First seen**: 2026-07-05

## Background Coding Agents: Predictable Results Through Strong Feedback Loops (Honk, Part 3)
- **Link**: https://engineering.atspotify.com/2025/12/feedback-loops-background-coding-agents-part-3
- **Description**: Background Coding Agents: Predictable Results Through Strong Feedback Loops (Honk, Part 3)
- **First seen**: 2026-07-05

## Background Coding Agents: Context Engineering (Honk, Part 2)
- **Link**: https://engineering.atspotify.com/2025/11/context-engineering-background-coding-agents-part-2
- **Description**: Background Coding Agents: Context Engineering (Honk, Part 2)
- **First seen**: 2026-07-05
